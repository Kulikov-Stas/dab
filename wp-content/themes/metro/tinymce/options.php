		...
		foreach($om_tmce_shortcode_options[$id]['options'] as $opt) {
			...
			switch ($opt['type']) {
				...
				case 'info':
					$out.= '
						<tr>
							<th colspan="2"><div class="howto">'.@$opt['desc'].'</div></th>
						</tr>';
				break;
				...
				case 'select':
					$out.= '
						<tr>
							<th style="width:25%"><strong>'.@$opt['title'].'</strong><div class="howto">'.@$opt['desc'].'</div></th>
							<td><select name="'.$opt['id'].'" id="'.$opt['id'].'">';
				...
				case 'color':
					$out.= '
						<tr>
							<th style="width:25%"><strong>'.@$opt['title'].'</strong><div class="howto">'.@$opt['desc'].'</div></th>
							<td>
								<div id="' . $opt['id'] . '_picker" class="colorSelector"><div></div></div>
								<input class="om-option-color" name="'. $opt['id'] .'" id="'. $opt['id'] .'" type="text" value="'. $opt['std'] .'" />
							</td>
						</tr>';
				break;
				...
				case 'icon':
					$out.= '
						<tr>
							<th style="width:25%"><strong>'.@$opt['title'].'</strong><div class="howto">'.@$opt['desc'].'</div></th>
							<td><style>.opt-icon-item{float:left;margin:0 10px 10px 0;padding:2px;border:2px solid transparent} .opt-icon-item img {display:block} .opt-icon-item.active {border-color:red}</style>
								';
					for($i=1;$i<=381;$i++)
					{
