		...
		switch ($field['type']) {
			...
			case 'text_browse':
				...
				if(floatval(get_bloginfo('version')) >= 3.5) {
				    ...
				    wp_enqueue_media();
					$output.='<a href="#" class="button insert-media add_media input-browse-button-wp5" rel="'.$field['id'].'" data-editor="content" title="'.__('Click &laquo;Insert Into Post&raquo; to choose the file','om_theme').'">'.__('Browse','om_theme').'</a>';
				} else
					$output.='<input type="button" class="button input-browse-button" rel="'.$field['id'].'" value="'.__('Browse','om_theme').'" title="'.__('Click &laquo;Insert Into Post&raquo; to choose the file','om_theme').'" />';
				$output.='
						</td>
					</tr>
				';
